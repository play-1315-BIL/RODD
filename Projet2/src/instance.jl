struct Instance
    ...
    function Instance(
        ...
        new(nb_parcels, area_min, area_max, budget, costs, areas, distances)
    end
end
...
function create_random_instance(edge_size::Int64)
    ...
    area_max = 2 + floor(rand() * (edge_size^2 - 2))
    area_min = 1 + floor(rand() * area_max)
    budget = max(area_min * 10, rand() * (edge_size^2 * 2))
