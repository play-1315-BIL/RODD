...
function graph_from_instance(instance::Instance)
    ...
    for (index1, sommet1) in enumerate(sommets)
        for (index2, sommet2) in enumerate(sommets)
            ...
            if sommet1[2:end] == (0, 0) && sommet2[2:end] == (0, 0) && l2 == min(l1 + 1, instance.l_max)
                # jachere -> jachere
                push!(delta_plus[index1], index2)
                push!(delta_plus[index2], index1)
                adj[index1, index2] = 1
            elseif sommet2 == (1, 0, 0) && j1 != 0
                # culture -> jachere
                push!(delta_plus[index1], index2)
                push!(delta_plus[index2], index1)
                adj[index1, index2] = 1
            elseif a2 == a1 + 1 && (j1 in instance.precedence[j2]) && l1 == l2
                # all -> culture
                push!(delta_plus[index1], index2)
                push!(delta_plus[index2], index1)
                adj[index1, index2] = 1
...
function create_instance()
    ...
    rendements::Array{Int64, 4} = zeros(l_max, a_max, C1 + C2 + 1, C1 + C2)
    ...
    return Instance(T, P, a_max, l_max, C1, C2, demand, precedence, rendements)
end
